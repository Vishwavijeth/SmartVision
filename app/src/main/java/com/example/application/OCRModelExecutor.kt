...
class OCRModelExecutor(
    ...
    private val ocrResultListener: OCRResult? = null
    ...
    init {
        try {
            ...
            val exceptionLog = "OpenCV not loaded: ${e.message}"
    ...
    fun execute(data: Bitmap): ModelExecutionResult {
        try {
            ...
            Log.d(TAG, "detected texts: ${detectTexts(data)}")

            //overlayOCR.setBoundingBoxInfo(boundingBoxInfoList, max(ratioWidth, ratioHeight))

            //viewModel.ocrResults.postValue(ocrResults)

            ocrResultListener?.onOCRResult(ocrResults)
    ...
    private fun detectTexts(data: Bitmap) {
        ...
        for (y in 0 until transposeddetectionScores[0][0].size) {
            ...
            for (x in 0 until transposeddetectionScores[0][0][0].size) {
                ...
                val textDetection = RotatedRect(center, Size(w.toDouble(), h.toDouble()), (-1 * angle * 180.0 / Math.PI))
        ...
        if(boundingBoxesMat.size() == detectedConfidencesMat.size()){
            NMSBoxesRotated(
                boundingBoxesMat,
            ...
            if(boundingBoxesMat.size() == detectedConfidencesMat.size()){
                ...
                Log.d(TAG, "Dimensions are right")
            }else{
                Log.d(TAG, "dimensions are not matched")
            }
        }else{
            Log.d(TAG, "dimensions are not matched")
        }
        Log.d(TAG, "Detected ${detectedRotatedRects.size} text regions:")
    ...
    private fun recognizeTexts(
        ...
        for (i in indicesMat.toArray())
        {
            ...
            if (recognizedText != "") {
                ...
                Log.d(TAG, "recognized texts : $recognizedText")
            }
        }
    ...
    interface OCRResult {
        fun onOCRResult(ocrResults: HashMap<String, Int>)
    }
